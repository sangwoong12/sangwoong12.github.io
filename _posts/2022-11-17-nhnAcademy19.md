...
- Collector는 Stream에서 종단연산에 해당되는 .collect() 함수의 파라미터에 해당하는 인터페이스이다.
  - ```stream().collect(Collectors.*)``` 다음과 같은 형태로 접근할 수 있다.
...
```
Optional<User> maxUser = users.stream().collect(Collectors.maxBy(Comparator.comparingInt(User::getUserAge)));
Optional<User> minUser = users.stream().collect(Collectors.minBy(Comparator.comparingInt(User::getUserAge)));
```

- summarizingInt, summarizingLong, summarizingDouble : 숫자의 합계 반환

```
int summing = users.stream().collect(Collectors.summingInt(User::getUserAge));
```

- averagingint, averagingLong, averagingDouble : 숫자의 평균을 반환

```
double average = users.stream().collect(Collectors.averagingInt(User::getUserAge));
```

- 숫자 통계 : count,min,max,sum,average 등

```
IntSummaryStatistics summarizing = users.stream().collect(Collectors.summarizingInt(User::getUserAge));
...
```
String joining = users.stream().map(User::getUserName) .collect(Collectors.joining(","));
```
...
```
var userMap = users.stream().collect(Collectors.partitioningBy(t -> t.getUserAge() > 30, Collectors.toList()));
```
